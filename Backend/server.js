require('dotenv').config(); const express = require('express'); const cors = require('cors'); const axios = require('axios'); const { OpenAI } = require('openai'); const PDFDocument = require('pdfkit'); const stream = require('stream');
...
try { const completion = await openai.chat.completions.create({ model: 'deepseek/deepseek-r1-0528-qwen3-8b:free', messages: [ { role: 'system', content: context }, { role: 'user', content: prompt } ], extra_headers: { 'HTTP-Referer': 'https://your-frontend.site', 'X-Title': 'SahilAssignmentAI' } }); const result = completion.choices[0].message.content; return { text: result, source: 'openrouter' }; } catch (error) { console.warn('⚠️ OpenRouter failed. Trying Gemini...', error.message); }
...
function createPDF(content, title = 'Generated Document', res) { const doc = new PDFDocument(); const bufferStream = new stream.PassThrough();

res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', attachment; filename="${title.replace(/\s+/g, '_')}.pdf");

doc.pipe(bufferStream);

doc.fontSize(16).text(title, { align: 'center', underline: true }); doc.moveDown(); doc.fontSize(12).text(content); doc.moveDown(2); doc.fontSize(10).fillColor('gray').text('Generated by SahilCodeLab', { align: 'right' });

doc.end(); bufferStream.pipe(res); }

// 🚀 Assignment Endpoint app.post('/generate-assignment', async (req, res) => { try { const { prompt, download } = req.body; if (!prompt) return res.status(400).json({ error: "Prompt is required" });

const context = `You are an expert academic content generator.
...
const result = await generateAIResponse(prompt, context); if (req.query.download === 'pdf') return createPDF(result.text, prompt, res); res.json(result); } catch (error) { res.status(500).json({ error: error.message }); } });
