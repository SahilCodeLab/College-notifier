require('dotenv').config(); const express = require('express'); const cors = require('cors'); const axios = require('axios'); const { OpenAI } = require('openai'); const PDFDocument = require('pdfkit'); // 📄 PDF support
...
// 📄 PDF Generator Function function generatePDF(content, res) { const doc = new PDFDocument(); res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', 'attachment; filename=output.pdf');

doc.pipe(res); doc.font('Times-Roman').fontSize(14); doc.text(content); doc.moveDown(); doc.fontSize(10).fillColor('gray').text('\nGenerated by SahilCodeLab', { align: 'center' }); doc.end(); }
...
Title Start with a suitable and formal assignment title based on the topic.

Table of Contents List of sections with titles (like a mini index).

Introduction (100-150 words) Brief overview of the topic. Why this topic is important or relevant. Objective or what this assignment will cover.

Main Body (400-600 words) 3 to 5 major points or subtopics. Each point should have: A clear heading Explanation in simple academic language Real-life examples or case studies Facts or data (if applicable) Use bullet points or numbering where needed.

Diagrams/Equations (Optional) If topic requires, include labeled diagrams or simple formulas (describe them if image not possible).

Applications (if applicable) Where or how this topic is used in real life or in academic/career fields.

Conclusion (100-120 words) Summarize key points covered. Add a thoughtful ending or personal insight related to the topic.

References Add 2-3 imaginary or common references like books, journals, or websites (no actual links required).

Guidelines: Format with Markdown headings (##). Keep content original, plagiarism-free, and academic. Suitable for school/college assignment submission. Maintain clarity, grammar, and formal tone. `.trim();
...
Clear section headings (##) Key concepts explained simply 2-3 relevant examples Practical applications if applicable`;
